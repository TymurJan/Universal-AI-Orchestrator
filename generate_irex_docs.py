...
def add_talan_letterhead(doc):
    ...
    run_req = p_req.add_run("ЄДРПОУ: 45119390 | р/р UA893052990000026001050211999 у АТ СЕНС БАНК, МФО 305299")
    ...
    run_addr = p_addr.add_run("Юридична адреса: м. Черкаси, вул. Смілянська, 44, оф. 212 | Тел: +38 (063) 656-89-99 | ngo.talan.ua@gmail.com")
...
def generate_main_proposal():
    ...
    add_bullet(doc, "Технічний архітектор (IT & AI Lead): ", "Забезпечує безперебійну роботу порталу, захист бази даних SQLite, налаштування серверної інфраструктури та алгоритмів маршрутизації.")
    ...
    add_bullet(doc, "Рішення 3 (Публічний PR): ", "Організація круглих столів та прес-конференцій із залученням регіональних ЗМІ для презентації досягнень платформи та легітимізації спільноти.")
    ...
    doc.save(OUT_DIR / "01_IREX_Grant_Proposal_NovyShlyakh_FULL_v3.docx")
    print("✅ Main proposal generated (v3).")
...
partners = [
    (
        "02_Letter_of_Support_Cherkasy_City_Council_v3.docx",
        "ІМ'Я ПРІЗВИЩЕ", 
    ...
    (
        "03_Letter_of_Support_DF_Cherkasy_TG_v3.docx",
        "ІМ'Я ПРІЗВИЩЕ",
        "Командир",
    ...
    (
        "04_Letter_of_Support_NGO_Cherkasy_City_Institute_v3.docx",
        "ІМ'Я ПРІЗВИЩЕ",
        "Голова правління",
    ...
    (
        "05_Letter_of_Support_NGO_Horizon_of_Change_v3.docx",
        "ІМ'Я ПРІЗВИЩЕ",
        "Голова організації",
    ...
    (
        "06_Letter_of_Support_Veteran_Pro_v3.docx",
        "ІМ'Я ПРІЗВИЩЕ",
        "Керівник простору",
...
def generate_budget_doc():
    ...
    doc.save(OUT_DIR / "07_IREX_Budget_Estimate_NovyShlyakh_v3.docx")
    print("✅ Budget doc generated (v3).")

if __name__ == "__main__":
    print("🚀 Starting generation of IREX Grant Package in _DROPZONE/OUT (v3 - ДСТУ 4163:2020)...")
    generate_main_proposal()
    for p in partners:
        generate_support_letter(p[0], p[1], p[2], p[3], p[4])
    generate_budget_doc()
    print("🎉 ALL DOCUMENTS GENERATED SUCCESSFULLY (v3)!")
