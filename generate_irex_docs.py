...
def generate_main_proposal():
    ...
    doc.save(OUT_DIR / f"01_IREX_Grant_Proposal_NovyShlyakh_FULL_{DOC_VERSION}.docx")
    print(f"✅ Main proposal generated ({DOC_VERSION}).")
...
partners = [
    (
        f"02_Letter_of_Support_Cherkasy_City_Council_{DOC_VERSION}.docx",
        "ІМ'Я ПРІЗВИЩЕ", 
        "Міський голова",
        "Черкаська міська рада",
        "Враховуючи стратегічне значення соціальної підтримки та адаптації наших захисників, Черкаська міська рада висловлює свою всебічну підтримку проєкту «Новий Шлях». Створення гібридного ветеранського хабу з цифровим порталом та штучним інтелектом розширить спроможність нашої громади щодо надання швидкої та безбар'єрної допомоги ветеранам та їхнім сім'ям."
    ),
    (
        f"03_Letter_of_Support_DSP_Cherkasy_{DOC_VERSION}.docx",
        "ІМ'Я ПРІЗВИЩЕ",
    ...
    (
        f"04_Letter_of_Support_NGO_Cherkasy_City_Institute_{DOC_VERSION}.docx",
        "ІМ'Я ПРІЗВИЩЕ",
        "Голова правління",
    ...
    (
        f"05_Letter_of_Support_NGO_Horizon_of_Change_{DOC_VERSION}.docx",
        "ІМ'Я ПРІЗВИЩЕ",
        "Голова організації",
        "Громадська організація «Горизонт змін»",
        "ГО «Горизонт змін» активно підтримує ініціативи, спрямовані на посилення спроможності громад та розвиток людського капіталу. Проєкт «Новий Шлях» є яскравим прикладом інноваційного підходу до вирішення складних соціальних викликів. Об'єднання понад 150 фахівців області в єдину верифіковану мережу створить потужний прецедент міжсекторальної взаємодії, і ми готові долучатися до навчальних та координаційних заходів проєкту."
    ),
    (
        f"06_Letter_of_Support_Veteran_Pro_{DOC_VERSION}.docx",
        "ІМ'Я ПРІЗВИЩЕ",
        "Директор закладу",
        "Комунальний заклад «Ветеранський простір «Ветеран про»»",
        "Як діючий комунальний ветеранський простір, ми щодня стикаємося з потребою у швидкій та якісній переадресації складних запитів ветеранів. Платформа «Новий Шлях», ініційована ГО «Талан ЮА», надає саме той цифровий диспетчерський інструмент, якого бракує офлайн-хабам. Ми готові об'єднувати наші зусилля за принципом «рівний — рівному», спільно вести кейс-менеджмент та надавати наш майданчик для проведення консультацій."
    ),
    (
        f"07_Letter_of_Support_TCC_SP_{DOC_VERSION}.docx",
        "ІМ'Я ПРІЗВИЩЕ",
        "Начальник обласного ТЦК та СП",
        "Черкаський обласний територіальний центр комплектування та соціальної підтримки (ТЦК та СП)",
        "Черкаський обласний ТЦК та СП висловлює повну готовність до взаємодії в межах проєкту «Новий Шлях». Спільна робота дозволить здійснювати первинне інформування військовослужбовців на етапі демобілізації та швидко направляти їх до бази перевірених фахівців. Ми впроваджуємо розповсюдження QR-кодів нашої цифрової диспетчерської безпосередньо у приміщеннях ТЦК для забезпечення швидкого доступу ветеранів до допомоги."
    ),
    (
        f"08_Letter_of_Support_Center_Employment_{DOC_VERSION}.docx",
        "ІМ'Я ПРІЗВИЩЕ",
        "Директор філії",
        "Черкаська філія Черкаського обласного центру зайнятості",
        "Черкаська філія Черкаського обласного центру зайнятості підтверджує стратегічне значення платформи «Новий Шлях» для адаптації ветеранів на ринку праці. Ми готові забезпечити оперативну інтеграцію наших програм безкоштовної перекваліфікації, навчання та профорієнтації з базою даних чат-бота, а також надавати актуальні вакансії від соціально активних роботодавців Черкащини для прямого доступу ветеранів у два кліки."
    )
]
...
def generate_budget_doc():
    ...
    doc.save(OUT_DIR / f"09_IREX_Budget_Estimate_NovyShlyakh_{DOC_VERSION}.docx")
    print(f"✅ Budget doc generated ({DOC_VERSION}).")

if __name__ == "__main__":
    print(f"🚀 Starting generation of IREX Grant Package in _DROPZONE/OUT ({DOC_VERSION} - ДСТУ 4163:2020)...")
    generate_main_proposal()
    for p in partners:
        generate_support_letter(p[0], p[1], p[2], p[3], p[4])
    generate_budget_doc()
    print(f"🎉 ALL DOCUMENTS GENERATED SUCCESSFULLY ({DOC_VERSION})!")
